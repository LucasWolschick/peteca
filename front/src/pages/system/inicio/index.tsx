...
export default function Index() {
  ...
  let todayTwoMonthsAhead = new Date(
    actualYear,
    actualMonth + 2,
    today.getDate()
  );
  if (actualMonth >= 10) {
    todayTwoMonthsAhead = new Date(
      actualYear + 1,
      actualMonth + 2,
      today.getDate()
    );
      ...
      <div className="container-fluid">
        <div className="row">
          <div className="col-lg-6">
            <h1 className="text-warning">Olá, {loggedUser?.user.nome}!</h1>
            ...
            <div className="row">
              ...
              <div className="col-lg-12 col-sm-6 mt-3 mt-sm-0 mt-lg-3">
                ...
                <label>
                  ...
                  {moment(todayTwoMonthsAhead).format("DD/MM/YYYY")}
                </label>
                {users
                  ?.filter(
                    (query) =>
                      new Date(
                        today.getFullYear(),
                        query.data_nascimento.getMonth(),
                        query.data_nascimento.getDate()
                      ) >= today &&
                      new Date(
                        today.getFullYear(),
                        query.data_nascimento.getMonth(),
                    ...
                    return (
                      <dl className="row">
                        <dt className="col-md-3">
                          {moment(
                            new Date(
                              user.data_nascimento.getTime() +
                                user.data_nascimento.getTimezoneOffset() * 60000
                            )
                          ).format("DD/MM")}
                        </dt>
                        <dd className="col-md-9">{user.nome}</dd>
                      </dl>
                    );
                  })}
              </div>
